...
---
= Exibir os volumes visíveis para o host no e-Series - Linux (NVMe sobre FC)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Para exibir informações sobre cada caminho NVMe para um volume e-Series, use o `nvme netapp smdevices [-o <format>]` comando.

A saída `<format>` pode ser normal (o padrão se -o não for usado), coluna ou json.
