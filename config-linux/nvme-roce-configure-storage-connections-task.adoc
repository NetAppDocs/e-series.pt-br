...
.Passos
. No painel esquerdo da interface do System Manager, selecione *Hardware*.
...
.O que se segue?
Após configurar as definições da porta NVMe sobre RoCE, link:nvme-roce-discover-connect-storage-host-task.html["Descubra e conecte o armazenamento no host"].
