...
summary: A ferramenta SMdevices, parte do pacote nvme-cli, permite visualizar os volumes atualmente visíveis no host. Esta ferramenta é uma alternativa ao comando nvme list. 
---
= Exibir os volumes visíveis para o host no e-Series - Linux (NVMe sobre RoCE)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Para exibir informações sobre cada caminho NVMe para um volume e-Series, use o `nvme netapp smdevices [-o <format>]` comando. O <format> de saída pode ser normal (o padrão se -o não for usado), coluna ou json.
...
----
.O que se segue?
Após visualizar os volumes visíveis, link:nvme-roce-setup-failover-host-task.html["Configure o failover no host"].
