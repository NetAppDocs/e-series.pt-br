...
.Antes de começar
Você deve estar executando o sistema operacional RHEL 9, RHEL 10, SLES 15 ou SLES 16 com o service pack mais recente compatível. Consulte o  https://mysupport.netapp.com/matrix["Ferramenta de Matriz de interoperabilidade do NetApp"^] para obter uma lista completa dos requisitos mais recentes.
...
NOTE: Se você estiver usando um NVIDIA HCA e desejar usar o driver DOCA externo, consulte o link:https://developer.nvidia.com/doca-downloads["Documentação NVIDIA DOCA"] para obter informações sobre como instalá-lo corretamente nesse ambiente.
...
NOTE: Scripts de rede não estão disponíveis para RHEL 10.
...
+
.O que se segue?
Após configurar o iniciador NVMe sobre RoCE no host, link:nvme-roce-configure-storage-connections-task.html["configurar as definições da porta NVMe sobre RoCE"].
