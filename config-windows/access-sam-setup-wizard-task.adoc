...
---
= Configure seu armazenamento usando o Gerenciador de sistema do SANtricity - Windows
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
.Sobre esta tarefa
Se for um utilizador iSCSI, certifique-se de que fechou o assistente de configuração durante a configuração do iSCSI.
...
* Não são detetados pools ou grupos de volume.
* Nenhuma carga de trabalho é detetada.
* Nenhuma notificação está configurada.


Se o assistente de configuração não for apresentado automaticamente, contacte o suporte técnico.
