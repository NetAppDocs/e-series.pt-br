...
* Duas redes separadas para alta disponibilidade. Certifique-se de isolar o seu tráfego iSCSI em segmentos de rede separados de outros tipos de tráfego, utilizando VLANs ou duas redes separadas.
...
.Sobre esta tarefa
Esta tarefa descreve como acessar a configuração da porta iSCSI na página Hardware. Você também pode acessar a configuração pelo menu:Settings[Sistema > Configurar portas iSCSI].
...
. No painel esquerdo, selecione menu:Hardware[Controllers and components].
