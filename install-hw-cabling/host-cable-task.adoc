...
NOTE: As seguintes informações não se aplicam a todos os sistemas de storage, para a fiação do host específica do seu sistema de storage, consulte link:../getting-started/getup-run-concept.html["Início rápido para e-Series"^] e clique no link para o seu sistema de storage específico.
