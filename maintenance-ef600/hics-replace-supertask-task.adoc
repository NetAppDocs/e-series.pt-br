...
.Sobre esta tarefa
Ao substituir um HIC com defeito, é necessário desconectar um controlador para colocar o sistema em um estado não redundante, substituir o HIC e reconectar o controlador.

.Antes de começar
* Revisão link:hics-overview-supertask-concept.html["Requisitos para atualizações HIC EF300 ou EF600"].
* Agende uma janela de manutenção de downtime para este procedimento. Você não poderá acessar os dados no array de storage até que este procedimento seja concluído com sucesso. Como ambos os controladores devem ter a mesma configuração HIC quando estão ligados, você deve desconectar um controlador para colocar o sistema em um estado não redundante ao instalar os HICs.
...
+
.. Selecione o menu: Hardware [Controllers and components].
